...
- (IBAction)addContact {
    ...
    // 2. 获取联系人模型
    RYContact *contact = [RYContact contactWithName:self.nameField.text PhoneNum:self.phoneField.text Youx:self.youxField.text];
...
- (void)viewDidLoad {
    ...
    [self.youxField addTarget:self action:@selector(textFieldDidChange) forControlEvents:UIControlEventEditingChanged];
}
...
- (void)textFieldDidChange {
    self.addBtn.hidden = !(self.nameField.text.length && self.phoneField.text.length && self.youxField.text.length);
}
